    ...
    : AudioProcessorEditor (&p), processor (p),  sonoLookAndFeel(p.getUseUniversalFont()), sonoSliderLNF(13), smallLNF(14), teensyLNF(11), panSliderLNF(12)
{
    ...
    mVideoButton->setTooltip(TRANS("Show the self-hosted video relay options"));
    mVideoButton->setTitle(TRANS("Video Relay"));
    mVideoButton->onClick = [this] {
        showVideoRelayView(true);
    };
...
void SonobusAudioProcessorEditor::showVideoRelayView(bool show, bool fromVideoButton)
{
    if (show && videoRelayViewCalloutBox == nullptr) {
        auto wrap = std::make_unique<Viewport>();
        Component* dw = this;
        const int defWidth = 500;
        const int defHeight = 230;

        if (!mVideoRelayView)
            mVideoRelayView = std::make_unique<VideoRelayView>(processor);

        const int prefwidth = jmin(defWidth, dw->getWidth() - 10);
        mVideoRelayView->setBounds(Rectangle<int>(0, 0, prefwidth, defHeight));
        const auto useheight = mVideoRelayView->getMinimumContentBounds().getHeight();
        const auto usewidth = std::max(prefwidth, mVideoRelayView->getMinimumContentBounds().getWidth());
        mVideoRelayView->setBounds(Rectangle<int>(0, 0, usewidth, useheight));

        wrap->setViewedComponent(mVideoRelayView.get(), false);
        mVideoRelayView->updateState();
        mVideoRelayView->setVisible(true);
        wrap->setSize(usewidth, jmin(useheight, dw->getHeight() - 24));

        const auto bounds = dw->getLocalArea(nullptr, fromVideoButton ? mVideoButton->getScreenBounds() : mMainLinkButton->getScreenBounds());
        videoRelayViewCalloutBox = &CallOutBox::launchAsynchronously(std::move(wrap), bounds, dw, false);
        if (auto* box = dynamic_cast<CallOutBox*>(videoRelayViewCalloutBox.get()))
            box->setDismissalMouseClicksAreAlwaysConsumed(true);
    }
    else {
        if (auto* box = dynamic_cast<CallOutBox*>(videoRelayViewCalloutBox.get())) {
            box->dismiss();
            videoRelayViewCalloutBox = nullptr;
        }
    }
}
...
void SonobusAudioProcessorEditor::showGroupMenu(bool show)
{
    ...
    items.add(GenericItemChooserItem(TRANS("Video Relay..."), {}, nullptr, true));
    ...
    auto callback = [safeThis,dw,bounds](GenericItemChooser* chooser,int index) mutable {
        ...
        if (index == 0) {
            ...
            // video relay
            safeThis->showVideoRelayView(true, false);
        }
        else if (index == 3) {
            // suggest new group
...
void SonobusAudioProcessorEditor::getCommandInfo (CommandID cmdID, ApplicationCommandInfo& info) {
    ...
    switch (cmdID) {
        ...
        case SonobusCommands::GroupLatencyMatch:
            ...
            break;
        case SonobusCommands::VideoRelayLink:
            info.setInfo (TRANS("Video Relay..."),
                          TRANS("Video Relay..."),
                          TRANS("Popup"), 0);
            info.setActive(currConnected && !currGroup.isEmpty());
            if (useKeybindings) {
...
void SonobusAudioProcessorEditor::getAllCommands (Array<CommandID>& cmds) {
    ...
    cmds.add(SonobusCommands::VideoRelayLink);
...
bool SonobusAudioProcessorEditor::perform (const InvocationInfo& info) {
    ...
    switch (info.commandID) {
        ...
        case SonobusCommands::GroupLatencyMatch:
            ...
            break;
        case SonobusCommands::VideoRelayLink:
            showVideoRelayView(true, mVideoButton->isShowing());
            break;
        case SonobusCommands::SuggestNewGroup:
...
PopupMenu SonobusAudioProcessorEditor::SonobusMenuBarModel::getMenuForIndex (int topLevelMenuIndex, const String& /*menuName*/)
{
    ...
    switch (topLevelMenuIndex) {
        ...
        case MenuGroupIndex:
            ...
            retval.addCommandItem (&parent.commandManager, SonobusCommands::VideoRelayLink);
