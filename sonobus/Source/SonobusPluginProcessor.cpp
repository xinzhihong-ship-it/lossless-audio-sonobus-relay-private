...
static String relayServerEnabledKey("RelayServerEnabled");
static String relayServerHostKey("RelayServerHost");
static String relayServerPortKey("RelayServerPort");
...
struct SonobusAudioProcessor::EndpointState {
    ...
    String relaySourceName;
    String relayGroupName;
...
static uint16 readRelayU16(const char *data, int offset)
{
    return (uint16)(((uint8)data[offset] << 8) | (uint8)data[offset + 1]);
}

static MemoryBlock makeSonoBusRelayPacket(const SonobusAudioProcessor::EndpointState& endpoint, const char *payload, int32_t payloadSize, uint8 packetType = 1)
{
    DynamicObject::Ptr header = new DynamicObject();
    header->setProperty("group", endpoint.relayGroupName);
    header->setProperty("source", endpoint.relaySourceName);
    header->setProperty("target", endpoint.relayPeerName);
    header->setProperty("directHost", endpoint.directIpaddr);
    header->setProperty("directPort", endpoint.directPort);

    String headerJson = JSON::toString(var(header.get()), true);
    MemoryBlock headerBytes(headerJson.toRawUTF8(), headerJson.getNumBytesAsUTF8());
    MemoryBlock packet;
    packet.setSize((size_t)SONOBUS_RELAY_HEADER_BYTES + headerBytes.getSize() + (size_t)payloadSize);

    auto *out = static_cast<char *>(packet.getData());
    memcpy(out, SONOBUS_RELAY_MAGIC, 4);
    out[4] = 1;
    out[5] = (char)packetType;
    writeRelayU16(out, 6, (uint16)headerBytes.getSize());
    writeRelayU16(out, 8, (uint16)payloadSize);
    memcpy(out + SONOBUS_RELAY_HEADER_BYTES, headerBytes.getData(), headerBytes.getSize());
    memcpy(out + SONOBUS_RELAY_HEADER_BYTES + headerBytes.getSize(), payload, (size_t)payloadSize);
    return packet;
}
...
static bool parseSonoBusRelayPacket(const char *packet, int32_t packetSize, String& group, String& source, MemoryBlock& payload)
{
    ...
    auto headerSize = readRelayU16(packet, 6);
    auto payloadSize = readRelayU16(packet, 8);
    auto expectedSize = SONOBUS_RELAY_HEADER_BYTES + (int32_t)headerSize + (int32_t)payloadSize;
    if (packetSize != expectedSize) {
        return false;
    }

    String headerJson = String::fromUTF8(packet + SONOBUS_RELAY_HEADER_BYTES, headerSize);
    auto parsed = JSON::parse(headerJson);
    if (!parsed.isObject()) {
        return false;
    }
    ...
    payload.replaceWith(packet + SONOBUS_RELAY_HEADER_BYTES + headerSize, payloadSize);
    return true;
}
...
static int32_t endpoint_send(void *e, const char *data, int32_t size)
{
    ...
    MemoryBlock relayPacket;
    const char *sendData = data;
    int32_t sendSize = size;
    if (endpoint->relayed) {
        relayPacket = makeSonoBusRelayPacket(*endpoint, data, size);
        sendData = static_cast<const char *>(relayPacket.getData());
        sendSize = (int32_t)relayPacket.getSize();
    }

    if (endpoint->peer) {
        result = endpoint->owner->write(*(endpoint->peer), sendData, sendSize);
    } else {
        result = endpoint->owner->write(endpoint->ipaddr, endpoint->port, sendData, sendSize);
    }
    ...
    if (result > 0) {
        ...
        DBG("Error sending bytes to endpoint " << endpoint->ipaddr << (endpoint->relayed ? " via relay" : ""));
    }
    return result;
}
...
})
{
    ...
    mVideoRelayClient = std::make_unique<VideoRelayClient>();
...
void SonobusAudioProcessor::restartVideoSender()
{
    if (!mVideoRelayClient)
        return;
...
void SonobusAudioProcessor::setRelayServer(const String & host, int port, bool enabled)
{
...
SonobusAudioProcessor::EndpointState * SonobusAudioProcessor::findOrAddEndpoint(const String & host, int port)
{
    ...
    for (auto ep : mEndpoints) {
        if (!ep->relayed && ep->ipaddr == host && ep->port == port) {
            endpoint = ep;
            break;
        }
    }
    ...
    for (int i = 0; i < n; ++i){
        ...
        case AOONET_CLIENT_CONNECT_EVENT:
        {
            ...
            if (e->result > 0){
                ...
                if (auto *parameter = mState.getParameter(paramAutoReconnectLast)) {
...
void SonobusAudioProcessor::getStateInformationWithOptions(MemoryBlock& destData, bool includecache, bool includeInputGroups, bool xmlformat)
{
    ...
    extraTree.setProperty(relayServerEnabledKey, mRelayServerEnabled, nullptr);
    extraTree.setProperty(relayServerHostKey, mRelayServerHost, nullptr);
    extraTree.setProperty(relayServerPortKey, mRelayServerPort, nullptr);
...
void SonobusAudioProcessor::setStateInformationWithOptions (const void* data, int sizeInBytes, bool includecache, bool includeInputGroups, bool xmlformat)
{
    ...
    if (tree.isValid()) {
        ...
        if (extraTree.isValid()) {
            ...
            bool relayEnabled = extraTree.getProperty(relayServerEnabledKey, mRelayServerEnabled);
            String relayHost = extraTree.getProperty(relayServerHostKey, mRelayServerHost);
            int relayPort = extraTree.getProperty(relayServerPortKey, mRelayServerPort);
            setRelayServer(relayHost, relayPort, relayEnabled);
