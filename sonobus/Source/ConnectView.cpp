...
publicGroupsListModel(this)
{
    ...
    mRelayServerStaticLabel = std::make_unique<Label>("relayserverst", TRANS("Relay Server:"));
    configServerLabel(mRelayServerStaticLabel.get());

    mRelayServerEditor = std::make_unique<TextEditor>("relayserveredit");
    ...
    mServerConnectContainer->addAndMakeVisible(mRelayServerEnabledButton.get());
    mServerConnectContainer->addAndMakeVisible(mRelayServerStaticLabel.get());
    mServerConnectContainer->addAndMakeVisible(mRelayServerEditor.get());
...
void ConnectView::escapePressed()
{
    if (!mServerGroupEditor->hasKeyboardFocus(false)
        ...
        && !mRelayServerEditor->hasKeyboardFocus(false)
...
void ConnectView::updateState()
{
    ...
    updateRelayServerFieldsFromProcessor();
...
void ConnectView::updateLayout()
{
    ...
    servRelayBox.items.clear();
    servRelayBox.flexDirection = FlexBox::Direction::row;
    servRelayBox.items.add(FlexItem(servLabelWidth, minitemheight, *mRelayServerStaticLabel).withMargin(2).withFlex(0.25));
    servRelayBox.items.add(FlexItem(100, minitemheight, *mRelayServerEnabledButton).withMargin(2).withFlex(0).withMaxWidth(110));
    servRelayBox.items.add(FlexItem(172, minpassheight, *mRelayServerEditor).withMargin(2).withFlex(1));
    ...
    serverBox.items.add(FlexItem(100, minitemheight, servRelayBox).withMargin(2).withFlex(0).withMaxWidth(maxservboxwidth));
    ...
    minHeight = 5*minitemheight + 4*minpassheight + 58;
...
void ConnectView::applyRelayServerFields()
{
    ...
    const bool enabled = mRelayServerEnabledButton->getToggleState();
    processor.setRelayServer(host, port, enabled);
...
{
    mRelayServerEnabledButton->setToggleState(processor.getRelayServerEnabled(), dontSendNotification);

    String hostport;
    if (processor.getRelayServerHost().isNotEmpty()) {
        hostport << processor.getRelayServerHost();
        if (processor.getRelayServerPort() > 0) {
            hostport << ":" << processor.getRelayServerPort();
        }
    }
    mRelayServerEditor->setText(hostport, false);
...
void ConnectView::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == mRelayServerEnabledButton.get()) {
        applyRelayServerFields();
    }
