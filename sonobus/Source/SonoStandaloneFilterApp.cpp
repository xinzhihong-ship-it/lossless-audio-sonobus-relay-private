...
public:
    ...
    void handleCommandLine()
    {
        ...
        const String relaySpec("-r|--relay-server");
        const String relaySpecDesc("-r|--relay-server <address:port>");
        ...
        app.addCommand ({ relaySpec, relaySpecDesc,
            TRANS("Send peer audio packets through the specified relay server instead of directly to peers."), {},
            nullptr
        });
        ...
        auto relayserv = arglist.removeValueForOption(relaySpec);
        if (relayserv.isNotEmpty()) {
            relayServerHost = relayserv.upToFirstOccurrenceOf(":", false, true).trim();
        ...
        if (!doHeadless) {
            ...
            if (auto * sonoproc = dynamic_cast<SonobusAudioProcessor*>(pluginHolder->processor.get())) {
                ...
                if (doInitialConnect) {
                    ...
                    sonoproc->setRelayServer(relayServerHost, relayServerPort, relayServerEnabled);
