...
确认用户仍在对应 SonoBus group、授权未撤销、管理员已选择并开启设备。Windows 不会抢占摄像头或自动换到另一台：每次后台开启只尝试一次，失败后保持关闭，避免物理摄像头反复重连；关闭后需由管理员再次开启。客户端掉线或重新打开时，服务端会自动关闭摄像头授权。后台的分辨率、FPS、码率均为输出上限，只会向下调整。
