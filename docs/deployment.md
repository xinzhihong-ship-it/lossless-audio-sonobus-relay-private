...
- 管理员独占管理每组一路摄像头的开关和设备；Windows 物理摄像头采集使用 `SharedReadOnly`，不会抢占设备。虚拟 DirectShow 设备只有管理员显式选择后才会打开。
...
确认用户仍在对应 SonoBus group、授权未撤销、管理员已选择并开启设备。Windows 物理摄像头不会被抢占或静默换到另一台，只使用 `SharedReadOnly`；物理采集进程中断时可在同一设备上自动重连。管理员明确选择的虚拟 DirectShow 设备每次后台开启只尝试一次，失败后保持关闭，除非管理员改变状态或选择；虚拟设备不会自动作为 fallback。关闭后需由管理员再次开启。客户端掉线或重新打开时，服务端会自动关闭摄像头授权。后台的分辨率、FPS、码率均为输出上限，只会向下调整。
