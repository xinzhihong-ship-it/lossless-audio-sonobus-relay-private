...
视频媒体面完全自建：SonoBus 只轮询管理员控制状态；macOS 由 FFmpeg 采集，Windows 物理摄像头由独立 `SonoBusVideoCaptureHelper.exe` 以 `SharedReadOnly` 读取后交给 FFmpeg；只有管理员显式选择、且已识别为虚拟设备的 `dshow:` 摄像头才由随包的 `ffmpeg32.exe`（或适用的 64 位 FFmpeg）直接采集并发布 RTSP。虚拟摄像头不会自动作为 fallback。MediaMTX 负责 WebRTC/WHEP、RTSP 和观看者扇出。helper 崩溃或摄像头/网络中断不会进入音频回调，也不应拖垮 DAW。
...
- Windows 物理摄像头使用 `MediaCaptureSharingMode::SharedReadOnly` 和 `MediaFrameReader`，不会主动取得摄像头独占权；64 位 FFmpeg 只接收 NV12 帧、编码 H.264 并发布 RTSP。只有管理员明确选择的、已识别为虚拟设备的 `dshow:` 摄像头才按设备选择 64 位或 32 位 FFmpeg，兼容 32 位进程内滤镜；物理摄像头不走 DirectShow。
...
- 不再先用独立 helper 进程探测模式再重复打开摄像头；普通物理摄像头始终保留管理员选择的设备 ID，只使用 `SharedReadOnly`，采集进程中断时可在同一设备上自动重连，但不会静默换设备。管理员明确选择的虚拟 `dshow:` 设备每次后台开启只尝试一次；失败后相同管理员状态不会再次打开，除非管理员改变开关、设备或发布授权。客户端掉线或重新打开后自动恢复为关闭，必须由管理员再次开启。
- 私有 `molixiu-hook` 路径按"**真实摄像头优先**"取源：只要物理摄像头能被共享读取，就用物理摄像头；读不到（说明魔力秀正独占着摄像头、也就是它此刻在显示摄像头）才用 Hook 帧。这样魔力秀放视频/关闭摄像头时传的是真实摄像头画面，而不是它正在播放的内容。候选顺序是魔力秀最后选中的那台优先，其后是其余全部物理摄像头；被打不开或不出帧的摄像头记入 60 秒"忙"名单，后续优先尝试其它摄像头。
- 虚拟摄像头（YY开播/OBS/WebcastMate）**不再自动作为兜底**：它的内容取决于宿主程序，可能不是摄像头画面，会让管理员看到"有画面但画面不对"。需要时由管理员在下拉框里显式选择。
- 该 fallback 全程只用 `SharedReadOnly`，绝不取得独占权；并且每 30 秒把摄像头交还约 3 秒，让需要独占的软件（魔力秀、会议软件等）能拿到设备，之后自动重新取流。物理 `SharedReadOnly` 读取失败后停止当前候选；确认 Hook 帧恢复且当前 fallback 已释放后，再切回 Hook 帧。
- 管理页显示**实际在用**的摄像头名，而不是只显示管理员选择的那台；发布进程超过 15 秒没有进度输出会被回收并按管理员期望状态重连（仅适用于物理 `SharedReadOnly` 路径；显式选择的虚拟 `dshow:` 设备失败后不自动重连），不会保留假的"在线"状态。
