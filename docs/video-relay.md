...
- 魔力秀没有在输出摄像头画面时（关闭摄像头、播放视频、Hook 未注入），helper 自动改用一台**空闲物理摄像头**采集真实画面，而不是把魔力秀正在播的内容当成画面。候选顺序是魔力秀最后选中的那台优先，其后是其余全部物理摄像头；被打不开或不出帧的摄像头记入 60 秒"忙"名单，后续优先尝试其它摄像头。
- 如果所有物理摄像头都拿不到画面（典型情况：被 YY开播/会议软件独占），客户端自动改读已知的**虚拟摄像头**（YY开播、OBS、WebcastMate 等，DirectShow 采集），因为虚拟摄像头本身不需要独占物理设备；物理摄像头恢复可用后，下一次失败重试会切回。虚拟摄像头显示什么取决于它自己的宿主程序，因此它只是最后兜底，不替代物理摄像头优先。
- 该 fallback 全程只用 `SharedReadOnly`，绝不取得独占权；并且每 30 秒把摄像头交还约 3 秒，让需要独占的软件（魔力秀、会议软件等）能拿到设备，之后自动重新取流。Hook 帧一旦恢复，立即释放 fallback 并回到 Hook 画面。
- 管理页显示**实际在用**的摄像头名，而不是只显示管理员选择的那台；发布进程超过 15 秒没有进度输出会被回收并按管理员期望状态重连，不会保留假的"在线"状态。
