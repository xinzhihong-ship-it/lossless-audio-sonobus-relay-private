...
- `SharedReadOnly` 禁止修改摄像头格式。Windows 在该设备当前可共享的彩色源中选择分辨率×FPS 数据率最高的一路，默认原样编码；后台可设置更低的分辨率、FPS、码率上限，客户端只下采样或丢帧，绝不放大或补帧。
- 管理员选定的设备被独占、断开或共享模式失效时，客户端不会静默切换其他摄像头；后台保留下拉选择并显示原因，原设备按 1–30 秒指数退避重试。
