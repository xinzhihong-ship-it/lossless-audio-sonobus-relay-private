...
- 魔力秀路径按"**真实摄像头优先**"取源：只要物理摄像头能被共享读取，就用物理摄像头；读不到（说明魔力秀正独占着摄像头、也就是它此刻在显示摄像头）才用 Hook 帧。这样魔力秀放视频/关闭摄像头时传的是真实摄像头画面，而不是它正在播放的内容。候选顺序是魔力秀最后选中的那台优先，其后是其余全部物理摄像头；被打不开或不出帧的摄像头记入 60 秒"忙"名单，后续优先尝试其它摄像头。
- 虚拟摄像头（YY开播/OBS/WebcastMate）**不再自动作为兜底**：它的内容取决于宿主程序，可能不是摄像头画面，会让管理员看到"有画面但画面不对"。需要时由管理员在下拉框里显式选择。
- 该 fallback 全程只用 `SharedReadOnly`，绝不取得独占权；并且每 30 秒把摄像头交还约 3 秒，让需要独占的软件（魔力秀、会议软件等）能拿到设备，之后自动重新取流。物理摄像头一旦读不到，立即回到 Hook 帧。
