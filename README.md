...
- 每组最多一路摄像头；Windows 物理摄像头以 `SharedReadOnly` 选择当前可共享彩色源中数据率最高的一路，默认原样发布。已识别的虚拟摄像头只有在管理员明确选择后才走 DirectShow，且不会自动作为 fallback。管理员可持久化设置更低的分辨率、FPS 和码率上限；客户端只下采样或丢帧，绝不放大、补帧、抢占物理设备或静默换普通管理员选择的摄像头。
