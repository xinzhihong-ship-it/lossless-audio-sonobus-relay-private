...
- Physical Windows capture uses `MediaCaptureSharingMode::SharedReadOnly -> MediaFrameReader -> NV12 -> FFmpeg stdin`; FFmpeg does not open DirectShow for physical cameras. An explicitly selected known virtual `dshow:` device is the exception and uses the DirectShow runtime; virtual devices are never automatic fallback sources. The client never requests exclusive control for physical cameras, never silently switches away from the ordinary administrator-selected source-group ID, and rejects measured rates below 59 FPS instead of duplicating frames to claim 60 FPS.
- A busy, disconnected or sub-60 shared stream is returned in signed client status and shown in the admin row while the device selector remains available. Device enumeration uses exponential retry capped at 30 seconds; physical SharedReadOnly publisher recovery may retry the same selected ID, while an explicitly selected virtual DirectShow launch is one-shot until administrator state changes.
...
- Final source-native CI passed: Windows `31334269494`, ASIO `31334269486`, Linux client `31334269505`, macOS `31334269502` at `fc2370d`; Server `31352278652` at `a89df6e`. The first Server attempt exposed an existing bridge-test race, fixed by releasing its one-shot frame only after the WebSocket opened and adding a bounded receive timeout; five local 37/37 repetitions and the replacement CI passed.
- Final SHA-256: Windows `41bb96b82b31e910567a33fe43ff41beb2a38ee1eb6515d0334170d6c4566143`; ASIO `4cea9ca4a716da9eb1ac01a31700f6348d0943d0b77ddd64cef9a6c8539cdbda`; macOS pkg `13450779e7b06c9effdcb00288cc39dc03b9bed7785c9b3d0f5f9dae07c778e7`; Linux deb `d87f79f999258c9fea648f4614703f68023be661b97131a5d8dc8b9784e1bb9c`; Server bundle `3dc9246275982bad5d5c42e406ebc9e3bdb86567d1a0385540a7634805528293`.
- The repository was made permanently public by explicit administrator choice to use public GitHub-hosted Actions. Before the visibility change, 7,020 reachable Git objects and 5,864 text blobs were scanned; findings were limited to environment placeholders and test fixtures, with no private key or live GitHub/AWS/OpenAI/JWT credential detected.
