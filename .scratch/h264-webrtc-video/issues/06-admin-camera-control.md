...
- Administrators must select an exact reported camera before enabling capture; server validation rejects an empty device. Ordinary physical-camera failures stay attached to that ID and use only SharedReadOnly recovery; no virtual device is used as fallback. Explicit virtual DirectShow failures are one-shot until the administrator changes state or selection. The private MoLiXiu Hook route may use only physical SharedReadOnly fallback candidates. Persisted output caps may only reduce the selected source's resolution, FPS, or bitrate.
